...
def evaluate_midterm(
    project_dir: Path = DATA_SCIENCE_MIDTERM_DIR,
    ...
    supported_questions = schema_report.get("supported_questions",
                                            list(range(1, TOTAL_QUESTIONS + 1)) if compatible else [])
    blocked_questions = schema_report.get("blocked_questions",
                                          [] if compatible else list(range(1, TOTAL_QUESTIONS + 1)))
    profile_outputs = sorted(
        path.name for path in (TRENDYOL_PROFILE_DIR / "outputs").glob("*")
        if path.is_file() and path.name != ".gitkeep"
    )
    adapted_outputs = {"table_summary.csv", "missing_values.csv", "duplicate_summary.csv",
                       "categorical_summary.csv", "text_length_summary.csv", "schema_report.json",
                       "data_quality_report.json", "profile_summary.md"}
    adapted_complete = adapted_outputs.issubset(profile_outputs)
    technical_ready = bool(dataset) and adapted_complete and notebook.is_file()
    all_complete = technical_ready and bool(configured_colab)
    ...
    return {
        "id": "data_science_midterm",
        "name": "Trendyol Veri Kalitesi ve Keşifsel Veri Analizi",
        ...
        "completed_questions": 12 if adapted_complete else 0,
...
def evaluate_final_project(project_dir: Path = DATA_SCIENCE_FINAL_DIR) -> dict[str, Any]:
    model_ready = (
        TRENDYOL_PROFILE_DIR.parents[1]
        / "01-machine-learning/trendyol-search-relevance/models/trendyol_relevance_pipeline.pkl"
    ).is_file()
    return {
        "id": "data_science_final",
        "name": "Trendyol Search Relevance Intelligence",
        "category": "Veri Bilimi Final Projesi",
        "status": "available" if model_ready else "experimental",
        "dataset": "TEKNOFEST Trendyol 2026 Datathon",
...
def data_science_counts() -> dict[str, int]:
    ...
    completed = sum(
        1 for item in [midterm, final]
        if item["status"] in {"available", "verified"}
    )
    return {
        "assignments": 2,
        "completed": completed,
